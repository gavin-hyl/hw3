...
class LoRALinear(nn.Module):
    ...
    def __init__(self, base_layer: nn.Linear, rank: int, alpha: float) -> None:
        ...
        # Freeze the wrapped pretrained linear: only A and B will be trained.
        for p in self.base_layer.parameters():
            p.requires_grad_(False)

        d_in = base_layer.in_features
        d_out = base_layer.out_features
        # A: (rank, d_in) Kaiming-uniform; B: (d_out, rank) zeros so BA = 0 at init,
        # making the adapted layer start exactly equal to the base layer.
        self.A = nn.Parameter(torch.empty(rank, d_in))
        self.B = nn.Parameter(torch.zeros(d_out, rank))
        nn.init.kaiming_uniform_(self.A, a=math.sqrt(5))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # base_layer(x) + (alpha/rank) * (x A^T) B^T
...
def apply_lora_to_attention(model: nn.Module, rank: int, alpha: float) -> nn.Module:
    ...
    # Freeze everything first; the only trainable params will be the LoRA
    # adapters we add below (plus any task head the caller attaches afterwards).
